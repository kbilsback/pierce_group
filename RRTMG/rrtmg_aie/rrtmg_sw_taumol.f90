...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         if (jp(lay-1) .lt. layreffr .and. jp(lay) .ge. layreffr) &
            laysolfr = lay
         speccomb = colh2o(lay) + strrat*colco2(lay)
         !print*, "colh2o(lay): ", colh2o(lay)
         !print*, "colco2(lay): ", colco2(lay)
         !if (speccomb .eq. 0.) speccomb=1E-10
         specparm = colh2o(lay)/speccomb  !jkodros
         !if (specparm /= specparm) specparm=oneminus
         if (specparm .ge. oneminus) specparm = oneminus
         !print*, "SPECCOMB", speccomb
         !print*, "SPECPARM", specparm
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         speccomb = colh2o(lay) + strrat*colco2(lay)
         !if (speccomb .eq. 0.) speccomb=1E-10  !jkodros
         specparm = colh2o(lay)/speccomb
         !if (specparm /= specparm) specparm = oneminus
         if (specparm .ge. oneminus) specparm = oneminus
         specmult = 4._rb*(specparm)
