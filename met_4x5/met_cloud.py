...
for mn in range(0, len(start)): #iterate through 12 months
    ...
    for dy in range(0, days[mn]): #iterate through each day of the month
        ...
        fname = 'GEOSFP.' + str(date)+'.A3cld.4x5.nc' #change this
        print(fname)
        nc_file = Dataset(fname)
        nc_data = nc_file.variables['CLOUD'][:] # dont fortget to change this!
...
np.savez('clfrac_4x5_20160801.npz', clfrac=monthly_data) #change this
